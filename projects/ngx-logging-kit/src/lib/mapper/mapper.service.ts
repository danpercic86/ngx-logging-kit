...
export class NGXLoggerMapperService implements INGXLoggerMapperService {
    ...
    protected readonly sourceMapCache = new Map<string, Observable<SourceMap | undefined>>();

    /** cache for specific log position, key is the dist position, ie 'main.js:339:21' */
    protected readonly logPositionCache = new Map<string, Observable<INGXLoggerLogPosition>>();
    ...
    protected getStackLine(config: INGXLoggerConfig): string {
        ...
        const error = new Error("dummy error to get stacktrace, ignore it, it's just to get the stacktrace");
        if (!error.stack) {
            return null!;
        }
    ...
    protected getLocalPosition(stackLine: string): INGXLoggerLogPosition {
        ...
        if (positionEndIndex < 0) {
            positionEndIndex = stackLine.length;
        }
    ...
    protected getSourceMap(
        ...
        const distPositionKey = `${distPosition.fileName}:${distPosition.lineNumber.toString()}:${distPosition.columnNumber.toString()}`;
        ...
        if (!this.sourceMapCache.has(sourceMapLocation)) {
            if (this.httpBackend === null) {
                console.error(
                    "NGXLogger : Can't get sourcemap because HttpBackend is not provided. You need to import HttpClientModule",
                );
                this.sourceMapCache.set(sourceMapLocation, of(undefined));
            } else {
                ...
                this.sourceMapCache.set(
                    ...
                    this.httpBackend.handle(req).pipe(
                        ...
                        map((httpResponse: HttpResponse<SourceMap>) => httpResponse.body!),
                        retry(3),
                        shareReplay(1),
                    ),
                );
